...
| **Status extensions** | Success/warning color families beyond shadcn's lone `--destructive`, each with a fill, a foreground, a `-subtle` background tint, and a `-border` tint; `--warning-text` is a separate AA-safe text color from the warning fill | `--success`, `--success-subtle`, `--warning`, `--warning-text`, `--destructive-subtle` |
