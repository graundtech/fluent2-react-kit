...
- ~~**Resubmit to the shadcn Registry Directory once the logo is final.**~~ Resubmitted 2026-07-14 as [shadcn-ui/ui#11173](https://github.com/shadcn-ui/ui/pull/11173) (OPEN, MERGEABLE, awaiting maintainer review) — supersedes the withdrawn #11137. Backstory: #11137 (2026-07-11) shipped a placeholder "F2" monogram and was closed by the maintainer pending a final logo. The resubmission ships the **final single-color mark** — a monochrome reduction of `assets/brand/fluent2-react-kit-icon.svg` (rounded square + split-capsule token, `var(--foreground)`/`var(--background)`), chosen because `directory-list.tsx` renders every logo through `grayscale` + `*:[svg]:fill-foreground` at 32px, so a full-color logo would appear as a gray blob and multi-tone detail is lost — single-color is a rendering requirement, not a schema one (the zod schema only requires `logo: z.string()`). The registry passes all four directory requirements (open source; schema-valid `registry.json`; flat `/r/{name}.json` with all items reachable and live-verified HTTP 200; no `content` in the index) and the entry passes the `validate-registries.mts` zod schema. **Mechanics note for future upstream PRs:** the fork's `main` couldn't be synced (`gh` token lacks the `workflow` scope, and upstream had workflow-file commits), and the Git Data API refuses to create a ref to a commit whose ancestry isn't reachable from a fork ref. The working path was to base the branch on the fork's own (10-commits-behind) `main` via blob→tree(base_tree=fork tree)→commit(parent=fork main)→ref — the entry inserts between `@flowkit-ui` and `@fluid`, a region unchanged upstream, so the cross-fork compare is a clean +7/−0 single-file merge despite the stale base. To drop the "10 behind" (if a maintainer asks for a rebase): `gh auth refresh -s workflow` in an interactive terminal, sync the fork, and re-base the one commit. The old `feat/add-fluent2-react-kit-registry` branch (from #11137) still exists on the fork; the new one is `feat/registry-add-fluent2-react-kit`. See [`docs/registry.md`](registry.md#publishing-to-the-shadcn-registry-directory).
...
- ~~**`RadioGroupItem` dot vertical mis-centering (2026-07-14).**~~ Resolved — the checked brand dot sat ~3px high (browser-measured `verticalOffset: -3` on the 16px control; horizontal was fine). Root cause: `Radio.Root` renders a `display:block` `<span>` and the item never made it a flex container, so `Radio.Indicator` (a block-level flex box that stretches to the content width but is only as tall as the 8px dot) sat flush to the top of the circle — the `items-center`/`justify-center` lived on the *indicator* only, which merely centered the dot within its own 8px box, not within the ring. Fix: added `flex items-center justify-center` to the item Root so it centers the indicator on both axes (re-measured `0/0`; Figma checked-rest `9121:4036` confirms a centered dot). This was an isolated inconsistency — `Checkbox` (`checkbox.tsx:115` `inline-flex items-center justify-center`) and `Switch` (`switch.tsx:68` track `items-center`) already flex-centered their Roots, so the earlier appearance/token-focused Figma passes, which compared color/elevation rather than measuring geometry, missed it on Radio alone. Tests 12/12, typecheck clean. Folded into **Figma validation Pass 4** (below).
- ~~**Figma validation Pass 3 open follow-ups (filter-list) + geometry re-pass.**~~ Resolved as **Pass 4** (2026-07-14, [`docs/design/figma-visual-validation.md`](design/figma-visual-validation.md#pass-4--filter-list-follow-ups--form-control-geometry-re-pass-2026-07-14)) — the radio bug above prompted closing the two follow-ups Pass 3 left when the Figma MCP hit its rate limit, plus a new **geometry lane** across the form controls. Findings: (P4-M1) the radio dot fix above; (P4-M2) Multi Select chip ✕ hover was `hover:text-destructive` with no Fluent precedent → aligned to `hover:bg-accent hover:text-primary` matching Fluent Tag `.Secondary action` Hover (node `9112:11155`: brand glyph `#0f6cbd`==`--primary` over neutral fill `#ebebeb`≈`--accent`). Verified MATCH (no change): Checkbox glyph (0/0), Switch thumb (0/0, 3px symmetric inset), Input (32px/px-3/correct borders), chip in-field spacing (4px gap = `gap-1`, 4px radius), and Combobox (copies validated Input+Select recipes; the Fluent kit has no Combobox symbol — the page is Dropdown-only). 98 tests + typecheck green. The takeaway — **add indicator-vs-control offset measurement to every future validation pass** — is recorded in the Pass 4 doc.
