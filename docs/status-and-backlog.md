...
- **Progress `size`/large variant.** Fluent's ProgressBar ships a Medium (2px)/Large (4px) `Size` axis; this kit only offers the 2px Medium track, with no `size` prop. Surfaced by the Figma visual validation pass.
- **Alert status-extension hex precision.** `--warning-subtle`/`--warning-border`/`--warning-text` and `--destructive-subtle`/`--destructive-border` are close to, but not exact matches of, the Fluent Figma file's local Message-bar values (sourced from generic published Fluent ramp docs rather than this specific component instance) — e.g. `--warning-text` `#c43501` vs. the Figma instance's `#bc4b09`. Small, same-hue drift, not a contrast or functional issue. Surfaced by the Figma visual validation pass.
