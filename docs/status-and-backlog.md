...
- **Vercel deployment URL placeholder (3 coordinated spots).** `apps/demo/app/page.tsx` and `scripts/build-registry.mjs` each hardcode `REGISTRY_BASE_URL = "https://fluent2-react-kit.vercel.app"` as an independent constant — both must be updated together once a real production domain exists (see [`docs/registry.md`](registry.md#the-registry_base_url-placeholder-caveat) for exactly what each controls). Not wired to a shared env var today.
- **Submit to the shadcn Registry Directory** ([ui.shadcn.com/docs/directory](https://ui.shadcn.com/docs/directory)) once the above deployment URL is real and stable — see [`docs/registry.md`](registry.md#publishing-to-the-shadcn-registry-directory).
